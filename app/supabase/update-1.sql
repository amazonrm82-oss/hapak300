...
--  כשזה עבר — עבור ל-update-2.sql. חייבים את שניהם, בסדר הזה.
-- ═══════════════════════════════════════════════════════════════════════════


-- ───────────────────────────────────────────────────────────────────────────
-- patch-01-admin-rank.sql
-- ───────────────────────────────────────────────────────────────────────────
...
--
--  ⚠️ להריץ את patch-02 מיד אחרי: יומן הפעולות כאן רושם גם שינוי בנשק,
--  בפרופיל הרפואי ובמגבלות — עמודות ש-patch-02 מוסיף. הן נקראות רק כשמעדכנים
--  לוחם, כך שאין חלון אמיתי, אבל אין טעם להשאיר את השניים לא צמודים.
--
...
begin
  if not exists (
    ...
    execute $sql$
drop view if exists trainings_view cascade;
...
-- ═══════════════════════════════════════════════════════════════════════════
--  עדכון 2 — תקופות, פרטי לוחם, דו״חות וגיבוי
--
--  להריץ **אחרי** patch-01. להדבקה ב-SQL Editor של Supabase (New query →
--  Paste → Run). בטוח להריץ שוב ושוב, ולא נוגע בשום נתון קיים.
--
--  מה נכנס כאן:
--
--  1. נשק אישי ומספר נשק, פרופיל רפואי ומגבלות — לכל לוחם. מספר הנשק והפרופיל
--     נראים למפקדים וללוחם עצמו בלבד, ורק מפקד יכול לשנות אותם.
--
--  2. סגירת תקופה — התקופה שנסגרת נשמרת עם הנוכחות והדירוגים כפי שהיו ביום
--     האחרון, ונפתחת תקופה חדשה. עד היום שם התקופה פשוט נדרס.
--
--  3. דלי ‎backups‎ לגיבוי השבועי האוטומטי, שרק מנהל מערכת ומפקד החפ״ק קוראים.
-- ═══════════════════════════════════════════════════════════════════════════
...
--          ('weapon','weapon_serial','medical_profile','limitations');


-- ───────────────────────────────────────────────────────────────────────────
-- patch-03a-sadir-enum.sql
-- ───────────────────────────────────────────────────────────────────────────
